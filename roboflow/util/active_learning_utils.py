...
def count_comparisons(predictions, required_objects_count, required_class_count, target_class):
  if(len(predictions) < required_objects_count or 
     target_class and
     count_class_occurances(predictions, target_class) < required_class_count):
    return False
  else:
    return True
...
def check_box_size(prediction, minimum_size_requirement, maximum_size_requirement):
  if (prediction['height'] * prediction['width'] < maximum_size_requirement and
      prediction['height'] * prediction['width'] > minimum_size_requirement):
    return True
  else:
    return False
...
def clip_encode(image1, image2, CLIP_FEATURIZE_URL):
    ...
    if CLIP_FEATURIZE_URL == "CLIP FEATURIZE URL NOT IN ENV":
        raise Exception("You need to ad CLIP_FEATURE_URL to your env vars. To learn more about this active learning feature, contact Roboflow sales https://roboflow.com/sales. You can remove the similarity keys from your conditionals to use other active learning functionality.")
